...
parser.add_option("-d", "--domain", dest="domain", default="",
                help="ES Domain Name")
parser.add_option("-c", "--client", dest="client", default="",
                help="ES Client ID")
...
### Real code
metrics = {"CPUUtilization":{"type":"float", "value":None},"FreeStorageSpace":{"type":"float", "value":None}}
...
if options.metric in metrics.keys():
  ...
  try:
          res = conn.get_metric_statistics(Namespace="AWS/ES", MetricName=k, Dimensions=[
                {
                    "Name": "DomainName",
